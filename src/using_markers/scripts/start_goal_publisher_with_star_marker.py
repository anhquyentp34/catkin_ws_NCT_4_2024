...
class StartGoalPublisher:
    ...
    def publish_markers(self):
        while not rospy.is_shutdown():
            # Robot start point
            robot_start = self.create_star_marker(-7.0, 0.0, 0.0, "robot_start", 0, 0.0, 1.0, 0.0, 1.0)
            self.marker_pub.publish(robot_start)
            robot_start_text = self.create_text_marker(-7.0, 0.0, 0.0, "Robot Start", "robot_start_text", 10, 0.0, 0.0, 0.0, 1.0)
            self.marker_pub.publish(robot_start_text)

            # Robot goal point
            robot_goal = self.create_star_marker(5.0, 0.0, 0.0, "robot_goal", 1, 1.0, 0.0, 0.0, 1.0)
            ...
            # Person start point
            person_start = self.create_star_marker(-1.0, 7.0, 0.0, "person_start", 2, 0.0, 0.0, 1.0, 1.0)
            self.marker_pub.publish(person_start)
            person_start_text = self.create_text_marker(-1.0, 7.0, 0.0, "Person Start", "person_start_text", 12, 0.0, 0.0, 0.0, 1.0)
            self.marker_pub.publish(person_start_text)

            # Person goal point
            person_goal = self.create_star_marker(-1.0, -7.0, 0.0, "person_goal", 3, 1.0, 1.0, 0.0, 1.0)
            self.marker_pub.publish(person_goal)
            person_goal_text = self.create_text_marker(-1.0, -7.0, 0.0, "Person Goal", "person_goal_text", 13, 0.0, 0.0, 0.0, 1.0)
